...
export const EmployerDashboardPage: React.FC = () => {
  const [data, setData] = useState<IEmployerDashboard | null>(null);
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await axiosConfig.get("/dashboard/employer");
        setData(res.data);
      } catch (err) {
  ...
  const overviewData = [
    {
      name: "Overview",
      Gigs: data.totalGigs,
      Active: data.activeGigs,
      Applications: data.applicationsReceived,
    },
  ];
      ...
      <div className="p-6 bg-gray-100 min-h-screen space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <GradientCard
            ...
            value={`${data.averageStudentRating ?? "0"} `}
            icon={<FaStar size={28} />}
        ...
        <div className="bg-white p-6 rounded-xl shadow-md mt-10">
          ...
          <ResponsiveContainer width="100%" height={350}>
            <LineChart data={overviewData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="Gigs" stroke="#8884d8" strokeWidth={2} />
              <Line type="monotone" dataKey="Active" stroke="#00C49F" strokeWidth={2} />
              <Line type="monotone" dataKey="Applications" stroke="#ff7300" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
